...
<div class="column is-12 notification is-info" 
			v-else-if="products.length < 1 && !isSearching && !notFound">
	<span class="icon">
		<i class="fa fa-info"></i>
	</span>
	Selecione a categoria no menu.
</div>
<div class="column is-12 notification is-warning"
			v-else-if="notFound">
	<span class="icon">
	...
	<div class="price-label">
		<div class="price-title">
			<h1 class="title is-5">
				...
				<span>{{ product.acf.codigoprod }}</span>. {{ product.title.rendered }}
		...
		<div class="price-tag has-text-right">
			<p v-for="(price, tag) in product.acf" v-if="price.length != 0">
				<span class="size">{{ tag[tag.length - 1].toUpperCase() == 'U' ? '' : tag[tag.length - 1].toUpperCase() }} </span> 
